...
function buildWsUrl() {
  ...
  return `${protocol}://${host}${port}/ws/science_publishing/notifications/?token=${encodeURIComponent(token)}`
}

export function createNotificationsSocket({ onMessage, onError, onOpen } = {}) {
  ...
  socket.onmessage = (event) => {
    try {
      ...
      console.warn('Notifications WS parse error', e)
    }
  }
